...
__conda_setup="$('/goinfre/anunes-c/miniconda3/bin/conda' 'shell.zsh' 'hook' 2>/dev/null)"
if [ $? -eq 0 ]; then
    eval "$__conda_setup"
else
    if [ -f "/goinfre/anunes-c/miniconda3/etc/profile.d/conda.sh" ]; then
        . "/goinfre/anunes-c/miniconda3/etc/profile.d/conda.sh"
    else
        export PATH="/goinfre/anunes-c/miniconda3/bin:$PATH"
    fi
fi
unset __conda_setup
# <<< conda initialize <<<
